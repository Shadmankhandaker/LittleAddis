...
import 'package:love/EM/block/login_bloc.dart';
  ...
  void _apiCall(BuildContext context) {
    ...
    if (dropdownValue != "Select Language") {
      if (_emailController.text.isNotEmpty &&
        ...
        pr?.show();
        loginBloc
            .loginSink(_emailController.text, _passwordController.text)
            .then(
          (userResponse) async {
                ...
                if (value.success == true && value.statusCode == 200) {
                  if (dropdownValue == "English") {
                    SharedPreferences preferences =
                        await SharedPreferences.getInstance();
                    preferences.setString(
                        SharedPreferencesKey.Language, "English");
                  ...
                  pr?.hide();
                  errorDialog(context, "ተጠቃሚን ማረጋገጥ አልተቻለም");
                }
              });
            } else {
              pr?.hide();
        ...
        errorDialog(context, "እባክዎ ለመግባት የእርስዎን ምስክርነት ያስገቡ");
      }
    } else {
      pr?.hide();

      errorDialog(context, "እባክዎ ቋንቋ ይምረጡ");
    }
  }
  ...
  void getUserProfileData(int userId) {
    repo?.getProfileInfo(userId.toString()).then((value) async {
      if (value != null) {
        loginBloc.dispose();
        pr?.hide();
        errorDialog(context, "በመለያ ይግቡ ስኬት");
        ...
        child: new RaisedButton(
            ...
            onPressed: () {
              ...
              signInWithGoogle().whenComplete(() async {
                ...
                SharedPreferences preferences =
                    await SharedPreferences.getInstance();
                preferences.setString(SharedPreferencesKey.Language, "Amharic");
